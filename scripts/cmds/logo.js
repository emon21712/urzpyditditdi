...
Page 1 - 3`,
        ...
        case 3:
          return api.sendMessage(
            `LOGO LIST PAGE  3:❍ smoke\n\nPAGE 3 - 3`,
            threadID,
            messageID
          );
        default:
          return api.sendMessage(
            `⚠️ Invalid page number! Please use "list 1" or "list 2" or "list 3" to view the available logo lists.`,
            threadID,
            messageID
          );
      }
    ...
    if (args.length < 2) {
      return api.sendMessage(
        `⚠️ Invalid command format! Use: logo list logo list (page number) or logo (logo name) (text)`,
        threadID,
        messageID
      );
    }
    ...
    message = "here's the [𝑺𝑴𝑶𝑲𝑬] Logo created:";
      break;
    case "captainamerica":
      apiUrl = `https://faheem-vip-010.faheem001.repl.co/api/textpro/captainamerica?text=${text}&text2=Walex`;
    ...
    case "carbon":
      ...
      message = "here's the [𝑪𝑨𝑹𝑩𝑶𝑵] created:";
        break;
    case "choror":
      apiUrl = `https://faheem-vip-010.faheem001.repl.co/api/textpro/choror?text=${text}&text2=walex`;
