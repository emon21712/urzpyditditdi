  ...
  onStart: async function ({ api, args, message, event }) {
    ...
    try {
      ...
      if (threadInfo && threadInfo.participantIDs) {
        ...
        if (participantIDs.includes(userID)) {
          ...
          api.addUserToGroup(userID, supportGroupId, (err) => {
            if (err) {
              ...
              api.sendMessage(
                "I can't add you because your ID is not allowed message requests or your account is private. Please add me, then try again...",
                threadID
              );
            } else {
              api.sendMessage(
        ...
        console.error("Failed to get thread info:", threadInfo);
        api.sendMessage("Failed to get thread info. Please try again later.", threadID);
      }
    } catch (error) {
      console.error("Error in join command:", error);
      api.sendMessage("An error occurred while processing your request. Please try again later.", threadID);
    }
  },
};