...
aliases: ["ss"],
  ...
  onStart: async function ({args, message, event }) {
    ...
    if (args.length === 0) {
      message.reply(`invalid input⚠\ease follow:\p}screenshot <url> \\p}screenshot -g <text>.`);
      return;
    }
    ...
    if (args[0] === '-g') {
      if (args.length < 2) {
        message.reply(`invalid text input after -g Tag⚠\ase follow:\}screenshot -g YourText`);
        return;
      }
